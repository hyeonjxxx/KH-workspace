-- CREATE TABLE을 할 수 있는 권한이 없어서 문제 발생
...
-- 뷰 객체를 생성할 수 있는 CREATE VIEW 권한이 없기 때문에 발생하는 문제
CREATE VIEW V_TEST
AS SELECT * FROM TEST;

--------------------------------------------------------------------------------

-- KH계정의 테이블에 접근해서 조회할 수 있는 권한이 없기때문에 문제발생
...
-- KH계정의 테이블에 접근해서 삽입할 수 있는 권한이 없기 때문에 문제 발생
