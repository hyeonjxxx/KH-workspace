/*
    ...
    내가 지정한 테이블에 INSERT, UPDATE, DELETE와 같은 DML문에 의해 변경되었을 경우 (테이블에 이벤트가 발생했을 경우)
    자동(묵시적)으로 매번 실행될 내용을 정의해둘 수 있는 객체
    
    EX) 
    입출고에 대한 데이터가 기록(INSERT) 될 때 마다 해당 상품에 대한 재고수량을 매번 수정(UPDATE)해야될 경우
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE 후 탈퇴회원들만 따로 보관하는 테이블에 자동으로 INSERT시켜야 될 경우
    ...
    * 트리거 종류
    ...
    - ROW TRIGGER(행 트리거) : 해당 SQL문 실행될 때 마다 매번 트리거 실행
             ...
             => NEW     : AFTER INSERT(입력후 자료), AFTER DELETE(수정된 자료)
    ...
    * 트리거  생성 구문 
    ...
    DECLARE 
        변수선언;
    BEGIN
        해당위에 지정된 이벤트 발생시 묵시적(자동)으로 실행할 구문;
    EXCEPTION
        예외처리구문;
    END;        
*/

-- EMPLOYEE테이블에 새로운 행이 INSERT 될때마다 자동으로 메세지 출력하는 트리거 정의
...
VALUES('300','고지석', '860518-1234567', 'J7', SYSDATE);

INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE, HIRE_DATE)
VALUES('301','서달미', '880211-2234567', 'J7', SYSDATE);
...
--    PBRAND (자료형 : VARCHAR2(20) , 제약조건 : NOT NULL) => 브랜드명
--    PRICE (자료형 : NUMBER)                             => 가격
--    STOCK (자료형 : NUMBER , 제약조건 : NOT NULL, 기본값 : 10) => 재고
...
VALUES(SEQ_PCODE.NEXTVAL, '갤럭시12', '샘송', 140000, DEFAULT);

INSERT INTO TB_PRODUCT
VALUES(SEQ_PCODE.NEXTVAL, '아이폰12PRO', '사과', 130000, DEFAULT);
...
-- 어떤 상품이 어떤 날 몇개가 입출고 되었는지 기록하는 테이블
...
-- 입출고 이력번호로 적용시킬 시퀀스(SEQ_DECODE) 생성
CREATE SEQUENCE SEQ_DCODE
NOCACHE;

-- 1001번 상품이 오늘 날자로 10개 입고 될 경우
...
-- 1001번 상품이 오늘 날짜로 100개가 입고
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL,1001,SYSDATE,100,'입고');

-- 1003번 상품이 오늘날짜로 10개 출고
INSERT INTO TB_PRODETAIL
