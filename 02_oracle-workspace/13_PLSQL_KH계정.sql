/*
    ...
    다수의 SQL문은 한번에 실행 가능 (BLOCK구조)
    
    * PL/SQL 구조
    - 선언부 (DECLAER SECTION) : DECLAER로 시작, 변수나 상수를 선어 및 초기화 하는 부분
    - 실행부 (EXECUTABLE SECTION) : BEGIN로 시작, 실행할 SQL문 또는 제어문(조건문, 반복문) 등의 로직을 기술하는 부분
    - 예외처리부 (EXCEPTION SECTION) : EXCEPTION로 시작, 예외 발생시 실행항 구문을 기술해두는 부분
...
/*
    1. DECLARE 선언부
       변수 및 상수 선언해놓는 공간 (선언과 동시예 초기화도 가능)
       일반타입변수, 레퍼런스타입변수, ROW타입변수
       
    1_1) 일반타입변수 선언 및 초기화
         [표현법] 변수명 [CONSRTANT] 자료형 [:= 값];
...
/*
    1_2) 레퍼런스 타입 변수 선언 및 초기화 (현재존재하고 있는 특정 컬럼의 데이터타입으로 지정)
        [표현법] 
        변수명 테이블명.컬럼명%TYPE;
...
/*
    ...
    각 자료형으로 EMPLOYEE의 EML_ID, EMP_NAME, JOB_CODE, SALARY / DEPARTMENT의 DEPT_TITLE 컬럼들의 데이터타입을 참조하게끔
    
    대체변수로 입력한 사원명과 일치하는 사원들을 조회해서 각 변수에 대입 후 출력
*/    
...
-- 80이상은 '상', 60점 이상은 '중', 60점 미만은 '하' 처리한 후 GRADE 변수에 기록
...
BEGIN
    ...
    -- 사원입력받아 해당사번과 일치하는 사원의 사번, 사원명, 부서명, 근무국가코드 조회한 후 => EID, ENAME, DTITLE, NCODE담기
