...
-- 전 직원의 평균 급여보다 더 적게 받는 사원들의 사워명, 직급코드, 급여 조회
...
-- 노옹철 사원의 급여보다 더 많이 받는 사원들의 사번, 사원명, 부서명, 급여
...
-- 전지연과 같은 부서인 사원들의 사번, 사원명, 전화번호, 직급명 조회
...
-- 부서별 급여합이 가장 큰 부서 하나만을 조회 (부서코드, 급여합) 
...
/*
    2. 다중행 서브쿼리 (MUTI ROW SUBQUERY)
       서브쿼리의 조회 결과값이 여러행일 때
        ...
        - [NOT] IN 서브쿼리 : 여러개의 결과값 중에서 한대라고 일치하는 값이 있으면 / 없다면 이라는 의미
    
        ANY  : 하나라도
        - > ANY 서브쿼리 : 여러개의 결과값 중에서 "하나라고" 클 경우
                          여러개의 결과값 중에서 가장 작은 값보다 클 경우             
        - > ANY 서브쿼리 : 여러개의 결과값 중에서 "하나라고" 작을 경우
                          여러개의 결과값 둥에서 가장 큰 값보다 작을 경우 
        ALL : 모든                 
        - > ALL 서브쿼리 : 여러개의 결과값의 "모든" 값보다 클 경우
                          여러개의 결과값 중에서 가장 큰 값보다 클 경우
        - < ALL 서브쿼리 : 여러개의 결과값의 "모든" 값보다 자긍ㄴ 경우
...
-- 사원 < 대리 < 과장 < 차장 < 부장
-- 대리직급임에도 불구하고 과장직급의 급여보다 많이 받는 사원 조회 (사번, 이름, 직급명, 급여)
-->> 과장직급들의 급여 조회
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
-- 사원 < 대리 < 과장 < 차장 < 부장
-- 과장직급임에도 불구하고 모든 차장직급의 급여보다 많이받는 사원 조회 (사번, 이름, 직급명, 급여)
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '과장' 
...
FROM EMPLOYEE
WHERE EMP_NAME = '하이유';     -- D5부서의 J5직급코드
-->> 부서코드가 D5이면소 직급코드가 J5인 사원 조회
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
  AND JOB_CODE = 'J5';

-->> 위의 내용들을 한나의 쿼리문으로! (단일행 서브쿼리)
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE DEPT_CODE = (SELECT DEPT_CODE
                    FROM EMPLOYEE
...
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE   --> 결과값이 1행이긴 하지만 여러컬럼
                                FROM EMPLOYEE
                                WHERE EMP_NAME = '하이유');


-- 박나라 사원과 같은 직급코드, 같은 사수사번을 가진 사원 조회 (사번, 사원명, 직급코드, 사수사번)
...
/*
    4. 다중행 다중열 서브쿼리
       서브쿼리 조회결과값이 여러행 여러컬럼일 경우

*/
-- 각 지급별 최소 급여를 받는 사원들 조회 (사번, 이름, 직급코드, 급여)
...
-- 각 부서별 최고급여를 받는 사원 조회 (사번, 사원명, 부서코드, 급여)
...
/*
    5. 인라인 뷰 (INLINE-VIEW)
    FROM 절에 서브쿼리를 제시하는 거
    
    서브쿼리를 수행한 결과(RESULT SET)를 테이블 대신에 사용함!
    
*/
-- 보너스 포함한 연봉이 3000만원 이상인 사원 조회 (사번, 이름, 보너스포함연봉, 부서코드)
SELECT EMP_ID, EMP_NAME, (SALARY+ (SALARY* NVL(BONUS,0))*12) "포너스포함한연봉", DEPT_CODE
FROM EMPLOYEE
--WHERE 포너스포함한연봉 >= 30000000;
WHERE SALARY+ (SALARY* NVL(BONUS,0))*12 >= 30000000;


-->> 인라인 뷰 사용해보자!!
SELECT *
FROM (SELECT EMP_ID, EMP_NAME, (SALARY+ (SALARY* NVL(BONUS,0))*12) "포너스포함한연봉", DEPT_CODE
      FROM EMPLOYEE)
WHERE 포너스포함한연봉 >= 30000000;
...
--> ORDER BY로 정렬한 테이블을 가지고 ROWNUM 순번 부여 후에 ROWNUM <=5 출력
SELECT ROWNUM 순번, E.* /*EMP_NAME, SALARY*/
FROM (SELECT *
      FROM EMPLOYEE
...
/*
    6. 순위 매기는 함수
      ...
      - RANK() OVER(정렬기준) : 공동 1위가 3명일 경우 그 다음 순위는 4위로 하겠다.
...
FROM EMPLOYEE;
--> 공동 19위 2명 그뒤에 순위는 21위

SELECT EMP_NAME, SALARY, DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위
FROM EMPLOYEE;
--> 공동 19위가 2명일 때 그 뒤 순위는 무조건 20위

-- 상위 5명 조회
SELECT EMP_NAME, SALARY, RANK() OVER(ORDER BY SALARY DESC) 순위
FROM EMPLOYEE
-- WHERE 순위 <= 5 ;
WHERE RANK() OVER(ORDER BY SALARY DESC) <= 5;  --> 윈도우 함수는 WHERE절 기술 불가
